...
(function () {
    ...
    /**
     * Abre un modal con los detalles del cliente dueño del vehículo
     */
    window.openClienteDetailModal = async function (clienteId) {
        if (!clienteId) {
            showTableMessage('error', 'ID de cliente no válido');
            return;
        }

        try {
            // Obtener detalles del cliente
            const response = await fetch(`/Cliente/DetailPartial?id=${clienteId}`, {
                headers: { 'X-Requested-With': 'XMLHttpRequest' }
            });
            ...
            const existingModal = document.getElementById('cliente-detail-modal');
            if (existingModal) {
                existingModal.remove();
            }

            const modalHTML = `
                <div id="cliente-detail-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                    <div class="relative p-4 w-full max-w-2xl max-h-full">
                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                                ...
                                <button type="button" onclick="closeClienteDetailModal()" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white">
                            ...
                            <div class="flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b dark:border-gray-600">
                                <button onclick="closeClienteDetailModal()" type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Cerrar</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            if (modal) {
                // Intentar usar Flowbite si está disponible
                try {
                    if (typeof Modal !== 'undefined') {
                        ...
                        modalInstance.show();
                        window._clienteDetailModal = modalInstance;
                        return;
                    }
                } catch (e) {
                    console.log('Flowbite Modal no disponible, usando fallback');
                }
            ...
            showTableMessage('error', 'Error al cargar los detalles del cliente');
        }
    };
     ...
     */
    window.closeClienteDetailModal = function () {
        const modal = document.getElementById('cliente-detail-modal');
        if (!modal) return;

        // Intentar cerrar con Flowbite
        if (window._clienteDetailModal && typeof window._clienteDetailModal.hide === 'function') {
            window._clienteDetailModal.hide();
            window._clienteDetailModal = null;
        } else {
            // Fallback
            modal.classList.add('hidden');
        ...
        // Remover modal del DOM después de un delay para animación
        setTimeout(() => {
